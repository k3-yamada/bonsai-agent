...
- **235**: Plan A factcheck Trajectory Scope Expansion (env-gated all-trajectories、項目 234 真因解消、1271 passed)
- **236**: Plan A G-5a/b smoke + 第 2 層真因確定 (production AssistantMessage event emit 不在、3 度確証)
- **237**: Plan A AssistantMessage Event Emit Fix + G-6a/b 2/2 PASS (🎉 factcheck total=5 / conflicting=3 = Bonsai-8B fabricate 検出初成功 = Plan A 真効力初実証、1275 passed)
- **238**: Lab v20 KG-FactCheck harness Phase 1-3 + **Lab v20 Phase 5 起動 (PID 32568、ETA 2026-05-17 06:30〜11:30)**
- **239**: Pattern 1 regex dash 対応 (factcheck.rs `RE_IS_THE_OF` を Pattern 2 と統一、1278 passed、cargo build --release は Lab v20 完走後保留)
...
| v20 | KG-FactCheck (進行中) | TBD | Pearson r ≥ 0.3 ACCEPT 基準 | PID 32568、~10-15h wall |
...
### Plan A 系列完結 (項目 230 → 234 → 235 → 236 → 237 → 238 → 239)
...
- Lab v20 effectiveness 検証 background 進行中、ACCEPT/REJECT は Pearson r で判定
