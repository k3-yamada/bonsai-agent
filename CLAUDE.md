...
218. **Cerememory ADR-011 ReviewState port 完遂** (Plan B 完遂、TDD strict 5 phase / 3 commits、plan `.claude/plan/cerememory-review-state-v12-impl.md`): arxiv ADR-011 由来 Cerememory `co-r-e/cerememory` (MIT、commit `b08d201`、2026-05-08) `docs/adr/011-adaptive-review-and-freshness.md` 12 KB ADR を bonsai HeuristicStore に Freshness 軸として port、Strength (decay/durability) と Freshness (still-safe-truth) を構造的に分離。production default OFF (`BONSAI_REVIEW_ENABLED` env unset で観測動作完全互換)、env=1 で freshness gate + review scheduler 有効化。Lab v17 副次 finding (項目 215 「pool 成熟による陳腐化 inject 比率増加 = ADR-011 命題の実機実証」) を直接活用。(a) `src/memory/review.rs` 新規 (~340 行 incl test) = `ReviewStatus` enum 7 variants (Unknown/Current/Due/Stale/Superseded/NeedsEvidence/Pinned) + `as_db_str`/`from_db_str` (未知文字列→Unknown 復元、ADR-011 §"Data Model" backward-compat 要件) + `ReviewState` struct 9 fields (status / importance 0.5 / volatility 0.5 / freshness 1.0 / source_confidence Option / last_reviewed_at Option<DateTime<Utc>> / next_review_at Option / review_count u32 / stale_count u32) + `Default` impl + `is_review_enabled()` (`BONSAI_REVIEW_ENABLED=1`/`true`、項目 214 ERL toggle / 項目 217 decay toggle と env name 対称) + 4 純関数 = `estimate_volatility_from_category` (failure_recovery=0.7 / verification=0.5 / efficiency=0.3 / 未知=0.5) + `compute_next_review_at` (scale=`max(volatility * 4 + 1, 1.0)`、base/scale で volatility 高ほど短間隔: vol=1.0→base/5, vol=0.0→base/1) + `should_skip_for_freshness` (env-gate + freshness < threshold で skip) + `ReviewOutcome::apply_to` (Confirmed/StillCurrent/Stale/Superseded/NeedsEvidence 全 5 variant、freshness クランプ [0,1] + saturating_add 安全) + 9 unit tests (b) `src/db/schema.rs` SCHEMA_V12 = 9 ALTER TABLE + 1 CREATE INDEX (`idx_heuristics_next_review` で `review_tick` SELECT 最適化) + `SCHEMA_VERSION` 11→12 + Migration entry + `test_schema_v12_contains_review_state_columns` (c) `src/memory/heuristics.rs` 拡張 = `Heuristic` struct に `review_state: ReviewState` 追加 (`#[serde(default)]` で V10 legacy 後方互換) + `ReviewRowTuple` type alias (clippy `type_complexity` 抑制、9 列 SELECT 用) + `save` 拡張 = volatility/next_review_at 投入 (INSERT に 2 列追加) + `find_top_k_for_task` SQL を 11 列→20 列に拡張、review_state 復元 (RFC3339 parse with timezone) + `review_tick(now)` 実装 (env-gated、`next_review_at IS NOT NULL AND next_review_at <= ?1` で id 昇順 LIMIT 50) + `record_review(id, outcome, now)` 実装 (load 9 col → `outcome.apply_to(&mut state)` → `last_reviewed_at = now` + `next_review_at = compute_next_review_at(now, state.volatility, 2_592_000)` + `review_count++` saturating + UPDATE 6 col) (d) `src/agent/context_inject.rs::inject_heuristics` 拡張 = `FRESHNESS_THRESHOLD=0.35` const + `should_skip_for_freshness(&h.review_state, 0.35)` filter (env=disabled で should_skip 常に false = legacy 完全互換、ERL OFF 時は機構自体が短絡で freshness gate 無関係)、Phase 1 Red commit (本 commit chain Phase 1) = todo!() 5 件 + 15 tests (9 review + 4 heuristics + 2 inject) で 13 fail / 2 pass (Default + is_review_enabled) → Red 確証、Phase 2 Green = 全 todo!() 解消 + V12 migration + 全 15 test pass + 1 schema test 追加 = **1116 → 1132 passed** (+16 / clippy 0 / fmt 0 / 退行ゼロ、AgentFloor pre-existing 3 failed は別 plan 範囲外で benchmark.rs untouched 確証)、Phase 3 Refactor = docstring 整備済 + ReviewRowTuple type alias 抽出 (cleanup) + REVIEW_TEST_LOCK / REVIEW_INJECT_TEST_LOCK で env mutation race serialize (項目 214 同パターン)、Phase 4 Smoke = `t_review_tick_with_env_enabled_returns_due_rows_only` (fixed-clock fixture 1_700_000_000 で past/future 2 row 投入、env unset → 空 / env=1 → past id のみ) + `t_record_review_stale_decreases_freshness_and_increments_stale_count` (save → record_review(Stale) で freshness 1.0→0.7、stale_count 0→1、status='stale'、2 回目で 0.4/2) で **1132 → 1134 passed (+2 fixture)**、Phase 5 effectiveness は別 plan (Lab v19 候補、`BONSAI_REVIEW_ENABLED` ON/OFF paired t-test、Lab v17 副次 finding stability 軸 ON 優位の構造的解決を測定)、API 完全 additive (`pub` 7 個 / `pub(crate)` 4 個 / signature 変更ゼロ / 既存 caller 21 callsite 無変更)、license attribution = `src/memory/review.rs` 冒頭 + `src/db/schema.rs` SCHEMA_V12 docstring + 既存 `docs/THIRD_PARTY_LICENSES.md` (項目 217 で先行作成) を共用、production 動作変更 = env=1 で freshness gate + review scheduler 有効化、env unset で **既存挙動 100% 維持**、副次=本 plan の `ReviewState` + scheduler + freshness gate 機構は他 store (Skill / Experience / Vault / KnowledgeGraph) で再評価可能な汎用基盤、次=★ Phase G (Working memory cap 7±2、~0.5 day) / (将来) Lab v19 paired t-test (decay + freshness gate 両軸で ACCEPT 判定) / (将来) ERL 機構 dead-code 化 plan / (保留) Milvus + Cognee 記憶最適化 plan (本 session で 3 URL 着手したが 2 URL too-many-redirects、Plan B 完遂後に再開予定)
219. **Phase G Working Memory Cap Phase 1-4 完遂** (Cerememory `cerememory-store-working` 派生、TDD strict 5 phase / 1 commit、roadmap `cerememory-extension-roadmap-d-g.md` §5): Miller 7±2 を default cap として `Session.messages: Vec<Message>` に hard cap、項目 82 ContextOverflowGuard (token-burst-based) の上流で構造的に context 圧迫を予防。production default OFF (`BONSAI_WORKING_CAP_ENABLED` env unset で観測動作完全互換)、env=1 で cap 強制 + `BONSAI_WORKING_CAP=N` で override (default 9、最低 3)。**設計修正**: roadmap §5.2 は `LoopState::max_active_messages` を target としていたが LoopState には messages フィールド不在 (実際の messages は `Session` 側) → 本実装は `Session.messages` を target に修正。(a) `src/agent/working_memory.rs` 新規 (~310 行 incl test) = `is_working_cap_enabled` (env reading、項目 214 ERL toggle / 項目 217 decay toggle / 項目 218 review toggle と env name 対称) + `working_cap_from_env` (`BONSAI_WORKING_CAP` env、default 9、`MIN_CAP=3` 入力検証で env=0/1/2 による session 完全 evict crash 防止) + `priority_score` (System=100/Tool=50/Assistant=30/User=10 で eviction 順序決定) + `cap_session_messages(&mut Session, cap) -> usize` (env=disabled で常に 0 = legacy 完全互換 / env=1 で eviction policy: System 絶対 protected (R1 agent crash 防止) + 直近 3 件 Tool protected (R2 reasoning context 欠落防止) + 残り候補から priority 昇順 → index 昇順で evict / `std::mem::take` + filter で in-place 削減 / 戻り値 = evicted count) + 9 unit tests (5 純関数 + 4 cap_session_messages 統合) (b) `src/agent/mod.rs` +1 (`pub mod working_memory`) (c) `src/agent/agent_loop/core.rs` = import + `for iteration` loop 冒頭 (before_step middleware の直前) に `cap_session_messages(session, working_cap_from_env())` hook 追加、env unset で no-op (legacy 動作 100% 保持)、Phase 1 Red = 5 純関数 pass + 4 cap_session_messages todo!() panic = 5/4 split で TDD valid Red 確証、Phase 2 Green = `cap_session_messages` 実装 + agent_loop core.rs hook で全 9 tests pass = **1134 → 1143 passed (+9 / clippy 0 / fmt 0 / 退行ゼロ)**、AgentFloor pre-existing 3 failed は別 plan 範囲外で benchmark.rs untouched 確証、Phase 3 Refactor = docstring 整備済 + WORKING_CAP_TEST_LOCK で env mutation race serialize (項目 214/217/218 同パターン)、Phase 4 Smoke = 4 cap_session_messages tests で env-gate / System protect / Tool protect (直近 3 件) / lowest-priority eviction (User 古い順) の 4 path 動作実証、Phase 5 effectiveness は別 plan (Lab v22 候補、`BONSAI_WORKING_CAP_ENABLED` ON/OFF paired t-test で項目 82 fire 頻度 -50% 以上 + Δscore ≥ 0 の ACCEPT 判定)、API 完全 additive (signature 変更ゼロ / 既存 caller 無変更 / env unset で observable behavior 完全互換)、production 動作変更 = env=1 で session.messages を 9 件 cap、env unset で **既存挙動 100% 維持**、副次=本機構は HeuristicStore 不要・Cerememory 系統 4 store (Plan A decay / Plan B ReviewState / Phase G working cap) を bonsai に揃えた、次=Phase 5 Docs (本 commit) → ★ Milvus + Cognee + mem0ai 記憶最適化 plan 起票 (本 session 中の interrupt で task #6 として保留、Plan B + Phase G 完遂で Cerememory 系統 port 一段落、外部 RAG/graph/mem0 系統との比較で次期記憶設計検討) / (将来) Lab v22 paired t-test / (将来) Phase D-F (Emotional/MCP/Audit hashchain、roadmap §3-4)
