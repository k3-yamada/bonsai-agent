...
230. **Plan A KG-Grounded Hallucination Check Phase 1-4 完遂 + Phase 4 Smoke G-4a/b 実機 2/2 PASS (★★★ Zenn 記事 7 選 #7 + EidoGraph + 項目 228 graph fusion 起点、項目 230)** — `.claude/plan/kg-grounded-fact-check-impl.md` (14 KB / 226 行) TDD strict 5 phase Phase 1-4 + edge case test 完遂、累積 9 commits (handoff 05-15 §3 commits + 本 session G-4a/b smoke 確証): `src/memory/factcheck.rs` 新規 317 行 (Triple/FactCheckResult/FactCheckSummary + 3 fn `extract_triples_from_text`/`verify_triple_in_kg`/`is_factcheck_enabled` + `run_factcheck_pass` + 12 unit test) + `src/memory/graph.rs` +70 行 (`contains_triple` BFS distance / `find_conflicting_edges` 同 subj+pred 別 obj edge 列挙) + `src/observability/audit.rs` +21 行 (`AuditAction::FactCheck { total/matched/unknown/conflicting/mean_path_len/duration_ms }` variant + `as_str` arm) + `src/agent/experiment.rs` +80 行 (`run_factcheck_pass_lab` private fn + AgentHER hook 直前の env-gated factcheck pass block、`is_factcheck_enabled()` で short-circuit、non-fatal: persist 失敗で summary 返却継続)、env opt-in `BONSAI_KG_FACTCHECK_ENABLED=1` (Cerememory 三本柱 pattern、default OFF)、SCHEMA migration 不要 (audit_log table に additive variant)。**1245→1257 passed (+12 / clippy 0 / fmt 0 / 退行ゼロ)**: Phase 1 Red 6 件 + Phase 3 Refactor 2 件 (FactCheckSummary 集約 + run_factcheck_pass 統合) + edge case test 4 件 (lowercase reject / Pattern 2 独立 / Pattern overlap なし / subject 既知 + object 未知 = Unknown)。設計 = **案 B Post-hoc Lab metric 採用** (案 A 全 LLM pre-emit / 案 C MCP server を REJECT)、failed trajectory のみ rule-based regex extraction で overhead 最小化、KG 未収載は false positive 回避のため `Unknown` 分類、AssistantMessage event_data の `{"content": "..."}` JSON parse 失敗時 raw string fallback。**Phase 4 Smoke G-4a/b 本 session 実機 (2/2 PASS、G-4c は hallucination task 未実装で defer)**: **G-4a (env unset、smoke=1、wall ~22 min)** = `[INFO][lab.factcheck]` log emit **0 件** (`is_factcheck_enabled()=false` で short-circuit) + bonsai.db audit_log `action_type='factcheck'` 行追加 **ゼロ** (G-4a 前 9237 行 → G-4a 後 9237 行不変 ✓)、experiment 1 件 pre-screen REJECT (delta=-0.1633 「ツール使用前に思考を強制」項目 47 重複)、AgentHER `failed=5 successful=2 relabels=2 skills=0 insights=2`。**G-4b (BONSAI_KG_FACTCHECK_ENABLED=1 + smoke=1、wall ~22 min)** = `[INFO][lab.factcheck] FactCheck post-Lab: total=0 matched=0 unknown=0 conflicting=0 mean_path_len=0.00` emit **1 行** + bonsai.db audit_log id=**9370** row 1 件追加 (action_data=`{"type":"FactCheck","total":0,"matched":0,"unknown":0,"conflicting":0,"mean_path_len":0.0,"duration_ms":17}`、**duration_ms=17 = factcheck pass overhead 17ms 極小確証**)、副次 = experiment 1 件 **ACCEPT (score=0.5140 / delta=+0.0179、Lab 天井 8 連続中の初 ACCEPT、ただし wiring 効果ではなく mutation noise 寄り、smoke=1 k=3 noise band 内で Phase 5 Lab v20 paired t-test で本判定)**。**total=0 の解釈**: failed trajectory 8 件あったが triple regex (`X is the Y of Z` / `X is a Y`) が SMOKE 7 task で hit せず → plan §4 G-4c 「hallucination-inducing task で conflicting>=1 確認」の **必要性を実機 confirm** (副次 finding)。**env-gating + AuditLogger persist 完全動作確証**、production default OFF (env unset → 既存挙動 100% 互換)、wiring 経路 = `experiment.rs:1456-1477` factcheck-enabled judge → `run_factcheck_pass_lab(store, since_event_id)` (line 1590-1634) → `EventStore::extract_failed_trajectories_since_id` で AssistantMessage 収集 → `factcheck::run_factcheck_pass(&texts, &graph)` → `AuditLog::new(conn).log(None, &AuditAction::FactCheck{..})` で persist (non-fatal `let _ = audit.log(..)`)。Phase 5 effectiveness 検証は別 plan `lab-v20-kg-factcheck-effectiveness.md` (10.6 KB、Pearson r ≥ 0.3 paired t-test、~10-15h wall) で defer、Phase 4 G-4c は別 plan `hallucination-inducing-benchmark-task.md` (11.0 KB、benchmark.rs に 3 false-fact task + factcheck.rs に seed 関数追加、~2-3h) で defer。次=★★ hallucination task 実装 + G-4c 完遂 / ★★★ Lab v20 起動 (~10-15h wall、Plan A Phase 5 effectiveness paired t-test) / ★ Pearson r 自前実装 (Lab v20 比較 metric)


231. **Lab v19 Frontier Effectiveness 完走 + score 軸 REJECT 確定 + 案 D 観測 partial baseline + 案 A 再設計 plan 起票 (★★ Lab 天井 8 連続確定、第 6 軸 = context-length axis ON 群単独 partial baseline 確立、項目 231)** — Lab v19 paired (PID 61085 / 起動 2026-05-15 07:16 / 完走 2026-05-15 23:17、wall **15h 37min** = handoff 05-15 ETA 12-15h ぴったり、項目 215 Lab v17 と同等 wall)、`scripts/lab_v19_paired.sh` で 5 paired = 10 cycle (`BONSAI_FRONTIER_ENABLED=1 BONSAI_FRONTIER_INJECT_ENABLED=1` ON cycle vs env unset OFF cycle)、`scripts/lab_v19_paired_ttest.py` 実機実行で **score 軸 paired t-test = REJECT 確証**: ON mean=**0.7007** vs OFF mean=**0.6935** / **paired Δ=+0.0072 / t=+0.2827 (df=4) / one-sided p=0.4262** → ACCEPT 基準 (a) Δ≥+0.015 NG / (b) p<0.1 NG → **REJECT** = **Lab v8/v9/v10/v14/v15/v16/v17 + v19 の天井 8 連続確定** (handoff 05-15 §「次セッション ★ 優先順位」#1 完遂)。**副次 finding (重要、本 session 検出) = 旧 plan `.claude/plan/lab-v19-frontier-analysis-and-phase5.md` §1.2(b) variance 軸 ACCEPT 基準の二重前提崩壊**: (i) **OFF cycle frontier emit ゼロ** = `test_off_{1..5}.log` 全 5 cycle で `[INFO][lab.frontier]` 行ゼロ、SQLite `frontier_bucket_scores='[]'` (`is_frontier_enabled()=false` で `emit_frontier_log` 早期 return = 設計上の正常動作) (ii) **target bucket 3 [8K, ∞) サンプルゼロ** = 通常 benchmark suite (5-22 task) は context が 8K 到達せず、LADDER + extended でなければ populate 不能 → variance ratio (ON/OFF) 計算は構造的に不可能、target bucket 3 解析もデータ不足。判断 = **案 D「観測されたものだけ記録」採用** (案 B `bucket_variance.py` 実装 skip、案 C 完全 REJECT 拒否、案 A 再設計 plan は別途起票、本 session で 4 案検討)。**第 6 軸 = context-length axis ON 群単独 partial baseline 確立 (本 session 初観測、項目 229 Phase 4 後の真初データ)**: bucket 0 [0, 2048): mean=**0.894400** stdev=0.000000 (5/5 cycle deterministic) / bucket 1 [2048, 4096): mean=**0.700040** stdev=**0.032641** (5/5 cycle、1bit Bonsai-8B の cycle-wise stability 軸 baseline) / bucket 2 [4096, 8192): mean=**0.114100** stdev=0.120778 (3/5 cycle、noise 比率 ≈ mean) / bucket 3 [8192, ∞): N/A (0/5)、**bucket 0→1 gradient = -0.194360** (案 A plan ACCEPT 基準 -0.10 を **1.94x 超過 ✓** = 第 6 軸 evidence 確証) / bucket 1→2 gradient = -0.585940 (3 cycle 限定 weak)、**副次副次 finding** = ON cycle 全体 score std=**0.031066** vs OFF cycle std=**0.035326** (ON 群が微優位 = 項目 215 Lab v17 副次 finding と整合、frontier ON で stability 劣化しないことの追加 evidence)。**案 A 再設計 plan 起票** (`.claude/plan/lab-v19-on-only-bucket-gradient-analysis.md` 280 行、推奨度 ★★、~2-3h、production code 変更ゼロ): ON 群単独 bucket gradient + cycle stability に再焦点化、target bucket 1 (5/5 robust populated) で計測、TDD strict Phase 1-3 で 8 test (parse_bucket_line / aggregate / compute_gradient / accept_threshold etc.)。**Lab v19 完走後 SQLite experiments table 状態**: pre-screen REJECT 経路は experiments row を書かない設計確証 (Lab v19 期間内 row ゼロ、log のみで完結)。**handoff 05-15 §229 副次 finding (a) Pre-screen REJECT carry-over fix plan** (`.claude/plan/frontier-prescreen-carryover-fix.md` 8.8 KB、~1.6h active + 1.5h smoke wall) は本 session 評価 = helper caller 2 箇所 (line 1240 + 2117) で plan §2.3 「1 callsite」と差異検出、別 session で着手候補。次=★★ frontier prescreen carryover fix Phase 1-3 / ★★ 案 A plan Phase 1-3 (Lab v19 既収集データ最大活用) / ★★★ hallucination task + G-4c (項目 230 完遂) / ★★★ Lab v20 (項目 230 Phase 5)
