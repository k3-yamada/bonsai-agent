...
- **243**: 🎉 **Lab v21 副作用対処 + G-7c PASS = success_fact input 書換で大幅改善 (commit `ee4659e`)** = G-7b で検出した「Restate this」file_write 副作用に対し input を「Output one sentence in your reply」に統一書換 (5 task / +5-5 lines)、G-7c smoke で **`total=15 matched=12 unknown=0 conflicting=3 mean_path_len=1.00`** / score=0.7613 / pass@k=0.8889 / 47 min = **matched +50% (8→12) / score +40.6% (0.5415→0.7613)** + 副作用解消 (git status clean)、Bonsai-8B 1bit prompt 明確化で大幅性能改善実証 = Lab v21 paired 起動 ready (別 session ~15-20h wall)
- **244**: 🎉 **KG lint pass Phase 1-4 完遂 + G-8a smoke PASS (Karpathy LLM Wiki Lint パターン適用)** = 5 commits (`6184998` Phase 1 Red 6 test + `2df46e2` Phase 2 Green LintReport + `8aa9e72` Phase 3 SSOT/is_kg_lint_strict + `46f98eb` Phase 4 experiment.rs 配線 + `2995085` ephemeral KG fix) — Lab v21 kill (cycle 1 で 5h+、core 32 task 完走 60-100h 推定で kill) 後 `cargo build --release` (binary 16:22 fresh)、G-8a v1 smoke で **conflicting=1995 false positive 検出** (production KG 累積 entity の all_conflicting_edge_pairs 暴走) → ephemeral `MemoryStore::in_memory()` 経由で seed-only scope lint に修正、G-8a v2 smoke で **conflicting=0 / orphan=0 / case_variant=0 ✅ uncovered=2** (= halluc seed の意図的設計 = entity 名を expected_keywords に含めない tip-off 防止) + factcheck `total=14 matched=14 conflicting=0 mean_path_len=1.00` / score=0.4481 / 副作用ゼロ、Lab v21 / v22+ paired 起動前 sanity gate として運用 ready
