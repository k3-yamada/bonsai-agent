...
Mac M2 16GB上でllama-server HTTP API経由で推論。400テスト、58ソースファイル。
...
## Lab実機テスト結果（2026-04-14, Bonsai-8B 1bit, k=3）

- ベースライン: score=0.8679, pass@k=1.0, pass_consecutive=1.0（安定）
- 「思考強制」ルール: score=0.8401（-0.0278）→ REJECT（1ビットモデルに過剰指示は有害）
- 「計画強制」ルール: score=0.8679（±0.0000）→ ACCEPT（neutral）
