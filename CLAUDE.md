...
Mac M2 16GB上でllama-server HTTP API経由で推論。409テスト、58ソースファイル。
...
## Lab実機テスト結果（2026-04-14, Bonsai-8B 1bit, k=3, 10サイクル）

- ベースライン: score=0.8596, pass@k=1.0
- **唯一のACCEPT: 「計画強制」ルール（+0.025）→ score=0.8846**
- AgentParam変異は全REJECT → デフォルト値（iter=10, tools=5, retries=3）が最適
- pass@k<1.0: max_iter=12, max_tools=3, max_retries=5 → パラメータ増減は安定性を損なう
- 承認率: 10%（10実験中1承認）
- **→ 計画強制ルールをデフォルトプロンプトに昇格済み（v2）**
