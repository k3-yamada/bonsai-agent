...
- **264**: 🚨 T6 案 D-2 (MEMORY_AUG) Phase 2a 完遂 (TDD strict 4 commit + `src/agent/t6_memory_aug.rs` ~270 LOC + integration test、`BONSAI_T6_MEMORY_AUG=1` env-gated で T6 success trajectory Jaccard top-K inject) + **重大 finding**: G-T6-D-1 (AUGMENT+BUDGET=1) 0.7726 vs 項目 262 G-T6-2 (AUGMENT 単独) 0.8778 = **-12.0% destructive interaction** / G-T6-D-2 (triple stack) 0.6251 = **-19.1% REJECT**、全 smoke で `[prev:` 0 = 実 compaction prune 一切不発火確証、**H-A4 (measurement noise / hidden binary diff) 仮説確定**、env default OFF
