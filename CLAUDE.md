...
- **243**: 🎉 **Lab v21 副作用対処 + G-7c PASS = success_fact input 書換で大幅改善 (commit `ee4659e`)** = 「Restate this」file_write 副作用に対し input を「Output one sentence in your reply」に統一書換、G-7c smoke で **`total=15 matched=12 unknown=0 conflicting=3 mean_path_len=1.00`** / score=0.7613 / pass@k=0.8889 / 47 min = matched +50% / score +40.6% 大幅改善 + 副作用解消
- **244**: 🎉 **KG lint pass Phase 1-4 完遂 + G-8a smoke PASS (Karpathy LLM Wiki Lint パターン適用)** = 5 commits (`6184998`/`2df46e2`/`8aa9e72`/`46f98eb`/`2995085`)、G-8a v1 smoke で **conflicting=1995 false positive 検出** → ephemeral `MemoryStore::in_memory()` 経由で seed-only scope lint に修正、G-8a v2 smoke で **conflicting=0 / orphan=0 / case_variant=0 ✅ uncovered=2** + factcheck `total=14 matched=14 conflicting=0` / score=0.4481 / 副作用ゼロ、Lab v21 / v22+ paired 起動前 sanity gate として運用 ready
- **245**: 🚨 **Lab v21 smoke 5-cycle paired 完走 REJECT (天井 10 連続) + structural metric finding 再現 + Ternary 真起動切替 + 4 件 plan/memory 起票** = (a) Smoke 15-task × k=3 × 10 cycle (wall **9h 33m**、~57 min/cycle、`lab-v21-smoke-logs/`) **REJECT 確定** = Pearson r=**+0.0000** (Lab v20 と同じ structural finding 再現: ON 5/5 で `(conf+matched+unknown)/total=1.0` deterministic → variance ゼロ)、副次 paired t-test Δscore=+0.0227 (p=0.2429 非有意、informational)、matched/total=0.79 平均 (G-7c 0.80 整合、variance 0.77〜0.81 で新 metric 軸候補) (b) **Ternary 真起動切替**: production config `~/Library/Application Support/bonsai-agent/config.toml` の MLX primary が起動忘れ運用だった (Lab v21 paired cycle 1=5h+ slowdown 原因)、`scripts/start-mlx-server.sh` 整備 + MLX server 起動済 (`prism-ml/Ternary-Bonsai-8B-mlx-2bit` port 8000)、Lab v22+ から本物の Ternary primary 経路で稼働 (c) **4 件起票**: `.claude/plan/vault-lint-coverage-check.md` (Vault lint、Karpathy LLM Wiki Lint Vault 軸) + `.claude/plan/dynamic-token-budget-compaction.md` (Dynamic Budget、Zenn 4 ratio 配分) + memory `ternary_bonsai_paths_2026_05_19.md` (Ternary 5経路調査 = MLX/GGUF×PrismML/stock/CLI) + memory `context_failure_modes_audit_2026_05_19.md` (Poisoning/Distraction/Confusion/Clash × 既存機構マッピング) + memory `fallback_chain_mlx_finding.md` (Lab v21 slowdown root cause = MLX 起動忘れ運用)
- **246**: 🎉 **Vault lint Phase 1-3 完遂 (TDD strict 3 commit、Karpathy LLM Wiki Lint Vault 軸拡張)** = Phase 1 Red (`1769193`): `src/knowledge/vault_lint.rs` 新規 + 5 failing test、Phase 2 Green + Phase 3 Refactor (`b51c00b`): 4 軸検出 (duplicate / stale / cross-cat / incomplete) + env getter (`is_vault_lint_strict()` / `vault_lint_stale_days()`) + `[INFO][lab.vault_lint]` warn_log prefix、orphan_entries は Phase 3+ で KG wiring 経由実装 (本 phase では常に空)。1294→**1307 passed (+13)** / clippy clean / fmt clean。Phase 4 (`experiment.rs::run_factcheck_pass_lab` 配線) は Phase A 完走後の cargo build --release 待ち
- **247**: 🟡 **Lab v22 Metric Redesign Phase A-D 実装 + CCG synthesis (進行中)** = (a) Phase B (`scripts/lab_v22_metric.py` 新規、Wilcoxon Signed-Rank exact + Cohen's dz + factcheck 補助 sanity gate + Pearson r 診断ログ + ACCEPT/REJECT 統合判定 + A/A mode、stdlib only) (b) Phase C (`BONSAI_LAB_TEMP` env、TDD strict 6 test + main.rs wiring、1300→1307 passed) (c) Phase A 起動 (`lab_v22_aa_test.sh`、両側 OFF×OFF + T=0 で σ_Δ noise floor 採取、09:38 開始 ETA ~14:38) (d) Phase D 起動 ready (`lab_v22_paired.sh`)。CCG synthesis (Codex 統計 + Gemini LLM eval): Pearson r 廃止 → paired Δscore + Wilcoxon + dz 主軸、smoke=10/decision=20/strict=27 cycle 推奨。補強 plan 3 件 (`lab-v22b-task-composition-halluc.md` / `lab-v22c-factscore-brier-metric.md` / `lab-v22d-attention-entropy-investigation.md`) 起票済。検証: Lab v21 smoke データ再評価で `mean Δ=+0.0227 PASS / dz=+0.4407 PASS / Wilcoxon p=0.1562 FAIL` = direction signal 検出可能、n=5 限界が明確化 (Phase E full lab で α=0.10 で 82% power 期待)
...
| v22 Phase A (進行中) | A/A test (noise floor σ_Δ 採取) | RUNNING | wall ~5h (T=0)、09:38 開始 ETA ~14:38 | Lab v22 metric redesign (Wilcoxon + dz + paired Δscore 主軸) の Phase D pilot 前提値、結果は別 session で評価 |
