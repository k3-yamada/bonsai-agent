...
- **257**: 🎉 **Z-3 Lightweight Drift Linter — Phase 1+2+3 完遂 + ECC critic HIGH 3 件 atomic fix (CCG synthesis 経由 Gemini 採用、Read-only 検出のみ)** = Zenn dragon1208/66547a030c0236 Step 8 適用、`.claude/plan/drift-monitor-weekly-gc.md` Lightweight 化 (フル GC 7h → 2-3h scope 適正化、production code touch ゼロ)。**CCG synthesis** = Codex (architecture) 推奨 B (項目 248 Phase 5 axis prune) vs Gemini (UX/strategic) 推奨 C Lightweight 案で意見分岐、Claude final = Gemini 採用 (Step 8 = analyst 評価 15% カバー最大 novel gap、Lab 多発の drift 蓄積仮説を Phase 2 cross-ref で機械検証可能化、Codex 推奨 B は次 session 自然進路として温存)。新規 scripts/drift/ (4 file): dead_code.sh (cargo +nightly udeps wrapper、graceful skip) + docs_sync.py (CLAUDE.md ↔ archive cross-ref + LAYER_ORDER sync 2 軸) + outdated.sh (cargo outdated wrapper、graceful skip) + run_lint.sh (orchestrator)、出力 `docs/quality/drift-YYYYMMDD.md`。**ECC code-reviewer atomic fix** (CRITICAL 0 / HIGH 3 解消): H-1 (silent regression bypass、claude_items 空 + CLAUDE.md > 100 行で FAIL 化) + H-2 (archive regex robustness、`^\s*(\d{3})[.)]\s+` で leading whitespace / tab / 全角空白 / paren-style 許容) + H-3 (dead_code.sh + outdated.sh standalone PWD 依存解消、PROJECT_ROOT cd 統一)。検証 = run_lint.sh All PASS (Phase 1 SKIP graceful + Phase 2 PASS + Phase 3 SKIP graceful) / /tmp standalone 実行も PROJECT_ROOT 経由で報告書一貫性。production code touch ゼロ。Gemini insight 反映 = Read-Only Drift Linter で Scaffolding > Model 原則の SSOT 確認、Lab REJECT root cause = stale context 仮説検証 trigger 機械化。後続 = Phase 4 (cargo llvm-cov coverage → docs/quality/scores.md upsert、~1.5h) + Phase 5 (Lab 完走 hook 統合、~30 min)
