...
- **242**: 🎉 **Lab v21 KG seed 拡張 Phase 1-4 完遂 (Lab v20 structural finding 解消の実機実証)** = Phase 1-3 (commit `8190889`): success_fact 5 task 追加 (benchmark.rs、Pattern 1 + Pattern 2 両軸 cover) + `seed_kg_for_factcheck_lab` 3→8 fact additive 拡張 (factcheck.rs)、1278→1286 passed (+8)。Phase 4 補完: SMOKE_TASK_IDS 10→15 拡張、G-7a PASS = env unset で factcheck emit=0 / score=0.5302 / 47 min (後方互換)、**G-7b PASS = factcheck `total=11 matched=8 unknown=0 conflicting=3 mean_path_len=1.00`** = matched 軸 variance 復活確証
