...
- **244 候補**: KG lint pass plan 起票 (planning-only、`.claude/plan/kg-lint-coverage-check.md`) = Karpathy LLM Wiki Lint パターン適用、`LintReport` 構造体 (conflicting/orphan/uncovered/case_variant) + `lint_kg_for_lab()` で paired 起動前の structural finding 事前検出経路確立、案 B (warning + opt-in strict gate) 推奨、TDD strict 5 phase + smoke G-8a/b/c 設計済
