#!/usr/bin/env python3
"""Lab v20 — KG-Grounded Hallucination Check effectiveness paired analyzer.

Plan: .claude/plan/lab-v20-kg-factcheck-effectiveness.md §3 Phase 5
入力: scripts/lab_v20_paired.sh が出力する `${LOG_DIR}/test_{on,off}_{1..5}.log`
出力: stdout に各 cycle factcheck summary + Pearson r + paired t-test (副次) + ACCEPT/REJECT 判定
...
def judge_accept(
    ...
    (a) Pearson r >= accept_r AND (b) ON 全 cycle で total >= 1。
    ...
    n_fired = sum(1 for s in on_summaries if s.get("total", 0) >= 1)
    if n_fired < len(on_summaries):
        reasons.append(
            f"(b) ON cycle で total>=1 件数 {n_fired}/{len(on_summaries)} NG (factcheck 不発火 cycle あり)"
        )
...
def main() -> int:
    ...
    ap.add_argument("log_dir", type=Path, help="lab v20 logs directory")
    ap.add_argument(
        "--accept-r",
        type=float,
        default=0.3,
    ...
    print(f"=== Lab v20 KG-FactCheck Effectiveness Analyzer (n_pairs={args.n_pairs}) ===")
    print(f"  {'pair':>4} {'on_total':>9} {'conflict':>9} {'unknown':>8} "
          f"{'on_score':>9} {'off_score':>9} {'delta':>8} {'fail_rate':>10}")
    print("  " + "-" * 4 + " " + "-" * 9 + " " + "-" * 9 + " " + "-" * 8 + " "
          + "-" * 9 + " " + "-" * 9 + " " + "-" * 8 + " " + "-" * 10)

    on_summaries: list[dict] = []
    on_failure_rates: list[float] = []
    on_conflict_unknown_rates: list[float] = []
    ...
    for i in range(1, args.n_pairs + 1):
        ...
        cu_rate = (conflict + unknown) / total if total > 0 else 0.0
        on_conflict_unknown_rates.append(cu_rate)
        ...
        print(f"  {i:>4} {total:>9} {conflict:>9} {unknown:>8} "
              f"{on_s:>9.4f} {off_s:>9.4f} {delta:>+8.4f} {fail_rate:>10.4f}")
    print()
    ...
    pearson = pearson_r(on_conflict_unknown_rates, on_failure_rates)
    accepted, reasons = judge_accept(on_summaries, pearson, accept_r=args.accept_r)

    print("=== Pearson Correlation (主 ACCEPT 軸) ===")
    print(f"  ON (conflict+unknown)/total: {on_conflict_unknown_rates}")
    ...
    print("=== ACCEPT 判定 (Plan A 系列 H_factcheck 採用判断) ===")
    print(f"  Pearson r >= {args.accept_r}: {pearson >= args.accept_r}  (got {pearson:+.4f})")
    n_fired = sum(1 for s in on_summaries if s.get("total", 0) >= 1)
    print(f"  ON 全 cycle で total>=1: {n_fired == len(on_summaries)}  "
          f"(got {n_fired}/{len(on_summaries)})")
    if accepted:
        print("  → ACCEPT (H_factcheck 採用、Lab v21+ で retry hook 試行検討)")
        return 0
    else:
        print("  → REJECT")
