...
if [ -z "$BONSAI_GGUF_PATH" ] && [ -z "$BONSAI_MODEL_GGUF_FILE" ]; then
    echo "エラー: BONSAI_MODEL_ID=$BONSAI_MODEL_ID には GGUF 既定がありません。BONSAI_GGUF_PATH または BONSAI_MODEL_GGUF_FILE を指定してください" >&2
    exit 1
fi
