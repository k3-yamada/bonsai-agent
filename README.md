...
- **269+のハーネスパターン** — 1ビットモデルの信頼性をスキャフォールディングで底上げ（1486 テスト、詳細は CLAUDE.md / [docs/quality/lab-history.md](docs/quality/lab-history.md)）
...
- **Phase 2 メモリ最適化 sidecar** — `start-mlx-sidecar.sh` で KV cache 量子化 (-71%) + `mx.set_cache_limit` 制御。M2 16GB で swap 阻止
