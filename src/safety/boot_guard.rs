...
impl BootGuard {
    ...
    pub fn should_safe_mode(&self) -> bool {
    ...
    fn save(&self) {
        let _ = std::fs::create_dir_all(self.state_file.parent().unwrap_or(&PathBuf::from(".")));
        let _ = std::fs::write(
            &self.state_file,
            format!("{{\"failures\":{}}}", self.failure_count),
        );
    }
    fn load_count(path: &PathBuf) -> u32 {
        std::fs::read_to_string(path)
            .ok()
            .and_then(|s| {
                let v: serde_json::Value = serde_json::from_str(&s).ok()?;
                v.get("failures")?.as_u64().map(|n| n as u32)
            })
            .unwrap_or(0)
    }
}
impl Default for BootGuard {
    fn default() -> Self {
        Self::new(3)
    }
}
...
mod tests {
    ...
    #[test]
    fn t_new() {
        let (g, _d) = test_guard(3);
        assert!(!g.should_safe_mode());
    }
    #[test]
    fn t_trip() {
        let (mut g, _d) = test_guard(2);
        g.record_failure();
        g.record_failure();
        assert!(g.should_safe_mode());
    }
