...
impl Vault {
    pub fn new(root: &Path) -> Result<Self> {
        ...
        for cat in &["decisions", "facts", "preferences", "patterns", "insights", "todos"] {
    ...
    pub fn append_all(&self, entries: &[StockEntry]) -> Result<usize> {
        let mut count = 0;
        for entry in entries { if self.append(entry).is_ok() { count += 1; } }
        Ok(count)
    }
...
mod tests {
    ...
    #[test] fn t_create() { let v = tmp_vault(); assert!(v.root().join("decisions.md").exists()); std::fs::remove_dir_all(v.root()).ok(); }
    #[test] fn t_append() {
        let v = tmp_vault();
        v.append(&StockEntry { category: StockCategory::Decision, content: "Rustを採用".into(), source: "s1".into() }).unwrap();
    ...
    #[test] fn t_dedup() {
        let v = tmp_vault();
        let e = StockEntry { category: StockCategory::Fact, content: "1ビットLLMは1.28GB".into(), source: "s1".into() };
        v.append(&e).unwrap(); v.append(&e).unwrap();
