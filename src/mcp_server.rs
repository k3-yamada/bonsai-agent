...
pub fn run_mcp_server(store: &MemoryStore) {
    ...
    for line in stdin.lock().lines().map_while(|r| r.ok()) {
        ...
        let resp = match method {
            "initialize" => {
                serde_json::json!({"jsonrpc":"2.0","id":id,"result":{"protocolVersion":"2024-11-05","capabilities":{"tools":{}},"serverInfo":{"name":"bonsai-agent","version":"0.1.0"}}})
            }
                ...
                let text = match tn {
                    ...
                    "get_vault" => {
                        ...
                        let cat = a
                            ...
                            .and_then(|v| v.as_str())
                            .unwrap_or("decisions");
                        std::fs::read_to_string(vp.join(format!("{cat}.md"))).unwrap_or_default()
                    }
                    _ => "unknown".into(),
                };
