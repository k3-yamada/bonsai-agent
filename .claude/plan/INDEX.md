...
| `gbrain-insights-port-impl.md` | Zenn 記事 「gbrain Knowledge Graph 設計」 (Y Combinator CEO 開発、TypeScript + PostgreSQL/PGLite + MCP) | **★ draft 設計再考必要** (session 05-11b §11 で `SearchResult.node_name` 不在 + memory↔graph bridging 未定義の major blocking gap 3 件発見、§13 参照) | 設計再考前は実装着手不可、scratch 再設計後 0.5 day |
| `frontier-benchmark-impl.md` | `antirez/ds4` `ds4-bench` (context frontier 毎の incremental throughput) ★ 新規 2026-05-14 | 6h | 第 6 軸 = context-length axis baseline 確立 (Lab v19 paired smoke で bucket [8K, 16K)+ variance 拡大 = 長文劣化検出能力獲得) |
| `greedy-on-protocol-impl.md` | `antirez/ds4` server "protocol greedy / payload sampled" 思想 ★ 新規 2026-05-14 | 4h | tool_call parse failure rate Δ ≤ -30% AND mean score Δ ≥ +0.01 (Lab v20 paired smoke) |
| `disk-backed-compaction-checkpoint-impl.md` | `antirez/ds4` "KV cache on disk first-class citizen" 思想 (既存 `ds4-insights-port-impl.md` は llama-server KV cache 配線、本 plan は agent LoopState snapshot で直交) ★ 新規 2026-05-14 | 5h | 中断後 resume で wall time −30% 以上 AND score 再現性 ±0.02 以内 (Lab v23 paired smoke) |

9 plan は production default OFF (env opt-in、項目 214 toggle pattern と一貫)。ds4 plan は 4 軸: Stage 1 (KV cache wiring、`ds4-insights-port-impl.md`)、Stage 2 (rax skill index、`ds4-rax-skill-index-impl.md`)、Stage 3 = 本 session 追加 3 plan (frontier benchmark / greedy-on-protocol / disk-backed compaction、antirez/ds4 2026-05-14 deep dive 由来) で独立。Stage 1-2 互いに独立着手可、Stage 3 の 3 plan は互いに直交かつ Stage 1-2 とも直交。gbrain plan は Stage 1 (Backlink Boost) のみ本 plan で完結、Stage 2 (Edge Provenance) は派生 plan で別 session 起票、Stage 3 (記憶層分離 validation) は Cerememory 三本柱で完遂済 = port 不要。Lab v17 完了後着手必須 (cerememory 3 plan)、ds4 Stage 1/2/3 + gbrain Stage 1/2 は独立着手可。
