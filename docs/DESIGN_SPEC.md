...
| **B2** | experiment.rs 統合（judge_threshold = 0.7） | ✅ 完了（931テスト、judge gate opt-in 配線） |
| **C** | ベンチマーク 22→40 タスク拡張（MultiFileEdit/LongRun/ToolChain/ErrorRecovery/McpInteg/Semantic/Reasoning/Summarization/Verification 各 ×2） | ✅ 完了（949テスト、+18タスク・項目163） |
| **D** | Workflow primitive（Sequential/Parallel/Loop trait） | ⛔ **見送り判定**（YAGNI 加重 1.0/17.0、`.claude/plan/phase-d-evaluation.md`） |
| **D-α** | SubAgentExecutor docstring に ADK Workflow primitive 対応を明記 | ✅ 完了（項目166、commit `728527b`） |
| **D-γ** | `run_experiment_loop` docstring に Phase D 評価結果へリンク | ✅ 完了（項目166、commit `728527b`） |
...
| `phase-d-evaluation.md` | ADK Phase D YAGNI 再評価ゲート（**見送り判定**、再評価トリガー 3 件定義） |
| `lab-v13-config-draft.md` | Lab v13 [experiment] セクション草稿（`judge_threshold=0.7`/`max_experiments=14`、推定 3360 ラン） |
