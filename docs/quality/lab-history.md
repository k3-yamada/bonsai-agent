...
| 268 G-PAIRED-263 (5/5 paired complete) | 案 263 Dynamic Budget ratio tune paired re-eval (BUDGET=0 vs 1、default 30/30/15/25) | **REJECT (paired evidence、Cohen's dz=-0.8620 medium-large)** | mean Δ=**-0.0683** (pair 1=-0.1873 / pair 2=-0.1037 / pair 3=0.0000 / pair 4=-0.0513 / pair 5=+0.0007、3 paired 明白 destructive + 2 paired ~0) / paired t=-1.93 / Wilcoxon p=0.9375 (destructive 方向 p≈0.063) / wall ~13h (cycle 平均 ~77 min) | 項目 263 G-DB-R-3 unpaired ACCEPT (+9.5%) を paired evidence で完全に覆す = H-A4 measurement noise + binary diff hidden coupling 確証。mean -6.83% は noise floor (~5%) 超過 = 真 destructive code path 仮説 (BudgetRatios 計算 overhead / allocated 伝播 latency / branch divergence) 支持。**`BONSAI_DYNAMIC_BUDGET` env default OFF 確定 (paired)**、production roll-out 不可、項目 248 Phase 5 axis-priority prune infrastructure (~180 LOC + 8 test) は max_context 縮小 future phase で実発火させる base として維持。Phase 6 plan (kg 25→30% micro-tune) は前提崩壊 + paired REJECT 二重否定で **保留 → 廃案推奨**。**Codex Harness Scaffolding>Model 原則再確認** = paired-evidence-driven cleanup pattern (項目 267 案 D-2 削除と同じ trajectory) 確立、項目 262 PROMPT_AUGMENT 最後の unpaired ACCEPT 真効果検証へ (g_paired_262_v2.sh next)。MLX server PID 77200 kill 完了。 |
| 269 G-PAIRED-262 (5/5 paired complete) | 案 262 T6 PROMPT_AUGMENT paired re-eval (AUGMENT unset vs =1、3 directive) | **REJECT (paired evidence、Cohen's dz=-0.4673)** | mean Δ=**-0.0480** (pair 1=+0.0014 / pair 2=+0.0707 / pair 3=-0.1560 / pair 4=-0.1560 / pair 5=0.0000) / paired t=-1.05 / Wilcoxon p=0.8125 (destructive 方向) / wall ~12h (cycle 平均 ~73 min、cycle_b_5 abort せず完走) | 項目 262 G-T6-2 unpaired ACCEPT (+14.4%) を paired evidence で完全に覆す = **unpaired ACCEPT 覆の paired REJECT 3 例目** (263 BUDGET / 264 MEMORY_AUG に続く)。on_score (AUGMENT=1) ~0.575 clustering vs baseline (off) 0.50-0.73 variance → delta は baseline 変動に支配される noise 構造 (pair 3/4 で baseline 0.7308 高、pair 2 で baseline 0.5048 低)。**`BONSAI_T6_PROMPT_AUGMENT` env default OFF 確定 (paired)**、ADR-003 (Paired Evidence 必須) 検証規律を 3 度実証。項目 261-269 で unpaired ACCEPT 3 件全て paired で覆る = 「Scaffolding > Model」原則下でも unpaired single-cycle は cherry-picked noise の決定的証拠群完成。 |
