...
- 当初 deferred とした robustness 項目のうち 2 件を後続で実装した:
  - **dup-content dedup** (実装済): `sync_file_chunks` の add ループに `seen` HashSet を
    追加し、同一ファイル内の content 完全一致段落を 1 度だけ保存するよう修正。
  - **IDF N+1 SQL** (実装済): `compute_idf_weights` を token 別 COUNT (N+1) から
    `COUNT(*)` + token 別 `SUM(CASE...)` の単一スキャンに batch 化 (df→idf 値は不変、
    characterization test で保証)。
- 以下は引き続き意図的に deferred (現規模で許容、変更は churn):
  - **`ingest.rs::unchecked_transaction()`**: 外側 tx 活性時に unsound だが、CLI(ingest) と
    agent-loop(save_session) は同一 store で interleave しない設計のため当該 path は発生しない。
    防御コード追加 / store API の `&mut` 化はいずれも「起こり得ない状況への対処」「churn」で
    あり、現状の意図的 RAII 設計 (`&self` のまま原子化) を維持する。
